...
export async function POST(request) {
  ...
  try {
    const { workout_id, content } = await request.json();

    if (!workout_id || !content) {
      return NextResponse.json({ error: 'Workout ID and content required' }, { status: 400 });
    }
    ...
    await db.prepare(`
      INSERT INTO comments (id, user_id, workout_id, content)
      VALUES (?, ?, ?, ?)
    `).run(id, session.user.id, workout_id, content);
